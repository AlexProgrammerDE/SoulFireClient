...
export default function CastMenuEntry() {
  ...
  useEffect(() => {
    ...
    void listen('cast-device-removed', (event) => {
      ...
      setDevices(devices => devices.filter(device => device.info.full_name !== payload.full_name))
    })
      ...
      <MenubarContent>
        {
          devices.length > 0 ? (
            devices.map((currentDevice) => (
              <MenubarItem key={currentDevice.info.id}
                           className={cn({
                ...
                toast.promise(invoke("connect_cast", {
                  ...
                  success: transportId => {
                    ...
                    setDevices(devices => devices.map(device => {
                      if (device.info.full_name === currentDevice.info.full_name) {
                        return {
                          ...device,
