...
export default function CommandInput() {
  ...
  const handleTabPress = async (text: string, element: HTMLInputElement) => {
    ...
    if (completionState.receivedCompletions === null || completionState.index === null) {
      ...
      const newIndex = (completionState.index + 1) % completionState.receivedCompletions.length
      completionStateNew = {
        lastWritten: text,
    ...
    if (completionStateNew.receivedCompletions!.length > 0) {
      ...
      split[split.length - 1] = completionStateNew.receivedCompletions![completionStateNew.index!]
  ...
  return <Input placeholder="Enter command" onKeyDown={handleKeyDown} onChange={(e) => {
    setCompletionState({
